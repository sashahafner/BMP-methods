...
\begin{itemize}
  \item $V$\textsubscript{{CH$_4$, $S, i, t$}}, the cumulative standardized volume of \ce{CH4} produced in bottle $i$ containing inoculum and substrate at time $t$ (mL)
  \item $V$\textsubscript{{CH$_4$, $I, j, t$}}, the cumulative standardized volume of \ce{CH4} produced in bottle $j$ with inoculum only (``blank'' bottle) at time $t$ (mL)
...
Typically relative standard deviation (RSD) is reported:
...
It may be more convenient to use directly calculate standard deviation values using available software functions, e.g., \texttt{STDEV()} in Microsoft Excel or \texttt{sd()} in R, instead of using Eqs.(\ref{eq:se1_calc}) and (\ref{eq:seI_calc}).
Then use the following two expressions to get standard errors.
...
The third (and optional) source of random error ($s_{\bar{x},3}$) is determined from the relative standard error in substrate VS content determination $rs_{\bar{x},VS}$ (dimensionless, expressed as a fraction of VS content).
