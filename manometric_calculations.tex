...
\end{equation}
Cumulative production is taken as the cumulative sum of interval values. 
Note that Eq. (\ref{eq:ch4m2}) reduces to Eq. (\ref{eq:ch4m1}) when $x_{CH_4, n} = x_{CH_4}$, i.e., if all flusing gas has been removed from the bottle headspace and biogas production is indeed only \ce{CH4} and \ce{CO2}.
